...
// In Prisma 7, we can override the datasource URL in the constructor using datasourceUrl
const prisma = new PrismaClient({
    datasourceUrl: process.env.DIRECT_URL,
});
const genAI = new GoogleGenerativeAI(process.env.GOOGLE_AI_API_KEY);
// Using text-embedding-004 as requested
...
async function seedRoles() {
    ...
    for (const roleData of roles) {
        ...
        // Upsert the role using findUnique/create/update to be explicit
        ...
        // Now update the embedding using raw SQL
        const embeddingStr = `[${embedding.join(",")}]`;
        await prisma.$executeRawUnsafe(`UPDATE "Role" SET content_embedding = $1::vector WHERE id = $2`, embeddingStr, roleId);
    }
}
